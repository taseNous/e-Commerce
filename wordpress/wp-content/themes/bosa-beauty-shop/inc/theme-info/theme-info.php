<?php
/**
 * Bosa Beauty Shop - Theme Info Admin Menu
 * @package Bosa Themes
 * @subpackage Admin
 */
if ( ! class_exists( 'Bosa_Theme_Info' ) ) {
    class Bosa_Theme_Info{
        ...
        public function prepare_class() {
            ...
            $this->page_title    = $this->theme_name . esc_html__( ' Info', 'bosa-beauty-shop' );
            $this->menu_title    = $this->theme_name . esc_html__( ' Setup', 'bosa-beauty-shop' );
                    ...
                    foreach ( $gs_steps as $gs_step ) {
                        ...
                        if( $is_gs ) {
                            if ( is_plugin_active('advanced-import/advanced-import.php') && is_plugin_active('keon-toolset/keon-toolset.php') && is_plugin_active('kirki/kirki.php') && is_plugin_active('elementor/elementor.php') && is_plugin_active('breadcrumb-navxt/breadcrumb-navxt.php') && is_plugin_active( 'yith-woocommerce-compare/init.php' ) && is_plugin_active( 'yith-woocommerce-quick-view/init.php' ) && is_plugin_active( 'yith-woocommerce-wishlist/init.php' ) && is_plugin_active( 'elementskit-lite/elementskit-lite.php' ) && is_plugin_active( 'woocommerce/woocommerce.php' ) && is_plugin_active( 'contact-form-7/wp-contact-form-7.php' ) && is_plugin_active( 'sina-extension-for-elementor/sina-extension-for-elementor.php' ) && is_plugin_active( 'bosa-elementor-for-woocommerce/bosa-elementor-for-woocommerce.php' ) ) {
                                echo ('<h3>'.esc_html__('Plugins added successfully. You have completed Step 1.', 'bosa-beauty-shop').'</h3>');
                            echo '</div></div>';
                            }else {
                                if ( ! empty( $title ) ) {
                                    echo '<h3>';
        ...
        public function recommended_actions() {
            ...
            if ( ! empty( $recommended_actions ) ) {
                        ...
                        if ( ! empty( $plugin_slug ) ) {
                            ...
                            switch ( $active['needs'] ) {
                                ...
                                case 'install':
                                    ...
                                    $label = esc_html__( 'Install', 'bosa-beauty-shop' );
                                    break;

                                case 'activate':
                                    $class = 'activate-now button button-primary';
                                    $label = esc_html__( 'Activate', 'bosa-beauty-shop' );

                                    break;

                                case 'deactivate':
                                    $class = 'deactivate-now button';
                                    $label = esc_html__( 'Deactivate', 'bosa-beauty-shop' );
                ...
                if ( $hooray ){
                    echo '<span class="hooray">' . esc_html__( 'Hooray! There are no recommended actions for you right now.', 'bosa-beauty-shop' ) . '</span>';
                    echo '<a data-action="reset" id="reset" class="reset-all" href="#">'.esc_html__('Show All Recommended Actions', 'bosa-beauty-shop').'</a>';
                }
                echo '</div>';
            }
        }
                        ...
                        if( !empty( $screenshot) ){
                            ...
                            if ( ! empty( $title ) ) {
                                ...
                                if ( ! empty( $download_link ) ) {
                                    echo '<a href="' . esc_url( $download_link ) . '" class="button button-primary install right">' . esc_html__( 'Download', 'bosa-beauty-shop' ) . '</a>';
                                }
                                if ( ! empty( $preview_link ) ) {
                                    echo '<a class="button button-secondary preview right" target="_blank" href="' . $preview_link . '">' . esc_html__( 'Live Preview', 'bosa-beauty-shop' ). '</a>';
                                }
                                echo '</div>';
                                echo '</div>';
                            }
...
$config = array(
    ...
    // Main welcome title
    'info_title' => sprintf( esc_html__( 'Welcome to %s - ', 'bosa-beauty-shop' ), $theme_name_config ),

    // Main welcome content
    'info_content' => sprintf( esc_html__( '%s is now installed and ready to use. We hope the following information will help and you enjoy using it!', 'bosa-beauty-shop' ), '<b>'.$theme_name_config.'</b>' ),
    ...
    'quick_links' => array(
        'theme_url'  => array(
            'text' => __( 'Theme Details', 'bosa-beauty-shop' ),
            'url' => 'https://bosathemes.com/' .$theme_name_config_url 
        ),
        'demo_url'  => array(
            'text' => __( 'View Demo', 'bosa-beauty-shop' ),
            'url' => 'https://demo.bosathemes.com/bosa/' .$demo_name_config_url
        ),
        'pro_url'  => array(
            'text' => __( 'View Pro Version', 'bosa-beauty-shop' ),
            'url' => 'https://bosathemes.com/bosa-pro'
        ),
        'rate_url'  => array(
            'text' => __( 'Rate This Theme', 'bosa-beauty-shop' ),
            'url' => 'https://wordpress.org/support/theme/' .$theme_name_config_url. '/reviews'
        ),  
    ),

    'tabs' => array(
        'getting_started'      => esc_html__( 'Getting Started', 'bosa-beauty-shop' ),
        'recommended_plugins'  => esc_html__( 'Recommended Plugins', 'bosa-beauty-shop' ),
        'support'              => esc_html__( 'Support', 'bosa-beauty-shop' ),
        'changelog'            => esc_html__( 'Changelog', 'bosa-beauty-shop' ),
        'free_pro'             => esc_html__( 'Free VS Pro', 'bosa-beauty-shop' ),
        'demos'                => esc_html__( 'Demos', 'bosa-beauty-shop' ),
        'rating'               => esc_html__( 'Rate Theme', 'bosa-beauty-shop' )
    ),

    /*Getting started tab*/
    'gs_steps' => array(
        'first' => array(
           'title' => esc_html__( 'Step 1 - To fully take advantage of the best our theme can offer, get started.', 'bosa-beauty-shop' ),
            'button_label' => sprintf(esc_html__( 'Get started with %s', 'bosa-beauty-shop' ), $theme_name_config ),
            'is_button' => true,
        ...
        'second' => array(
            'title' => esc_html__( 'Step 2 - Checkout starter sites (Demos)', 'bosa-beauty-shop' ),
            'desc' => esc_html__( 'All starter demos sites comes attached to Bosa just ready to pluck one from. Discover them, be enamoured and finally decide on the one you resonate with. If one doesnt catch your fancy dont be afraid to have a go at building a site with your own design.', 'bosa-beauty-shop' ),
            'button_label' => esc_html__( 'View Demo', 'bosa-beauty-shop' ),
            'button_link' => esc_url( 'https://demo.bosathemes.com/bosa/' .$demo_name_config_url ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'third' => array (
            'title' => esc_html__( 'Step 3 - Import demo of your choice', 'bosa-beauty-shop' ),
            'desc' => esc_html__( 'Once you have a found a site you like, you can move ahead and import it. It is to be noted that some necessary plugins will be installed to function the site as you see in the demo site. A fresh Wordpress site is recommended to mirror the demo site on your setup. Importing the demo on an already running site will create discrepancies with the site shown on the demo.', 'bosa-beauty-shop' ),
            'button_label' => esc_html__( 'Import Demo', 'bosa-beauty-shop' ),
            'button_link' => esc_url( admin_url( 'themes.php?page=advanced-import' ), 'bosa-beauty-shop' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'fourth' => array(
            'title' => esc_html__( 'Step 4 - Start editing content and making your site!', 'bosa-beauty-shop' ),
            'desc' => esc_html__( 'Once finishing the import you play with the demo to your hearts content.', 'bosa-beauty-shop' ),
        ),
        'fifth' => array (
            'title' => esc_html__( "Step 5 - That's it!", 'bosa-beauty-shop' ),
            'desc' => esc_html__( 'Go live with the website and enjoy it!', 'bosa-beauty-shop' ),
        ),
    ),
    ...
    'support_content' => array(
        'first' => array(
            'title' => esc_html__( 'Need more features?', 'bosa-beauty-shop' ),
            'desc' => esc_html__( 'Upgrade to PRO version for more exciting features and Priority Support.', 'bosa-beauty-shop' ),
            'button_label' => esc_html__( 'Upgrade to PRO', 'bosa-beauty-shop' ),
            'button_link' => esc_url( 'https://bosathemes.com/bosa-pro' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'second' => array (
            'title' => esc_html__( 'Go to Customizer', 'bosa-beauty-shop' ),
            'desc' => esc_html__( 'All Settings, Header & Footer Options, Theme Options, Widgets and Menus are available via Customize screen.', 'bosa-beauty-shop' ),
            'button_label' => esc_html__( 'Go to Customizer', 'bosa-beauty-shop' ),
            'button_link' => esc_url( admin_url( 'customize.php' ) ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'third' => array(
            'title' => esc_html__( 'Documentation', 'bosa-beauty-shop' ),
            'desc' => esc_html__( 'Please check our full documentation for detailed information on how to Setup and Use Bosa.', 'bosa-beauty-shop' ),
            'button_label' => esc_html__( 'Read full documentation', 'bosa-beauty-shop' ),
            'button_link' => esc_url( 'https://bosathemes.com/docs/bosa' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'fourth' => array (
            'title' => esc_html__( 'Support Forum', 'bosa-beauty-shop' ),
            'desc' => esc_html__( 'Got theme support question or found bug? Simply create a support ticket with your queries and we will get back to you.', 'bosa-beauty-shop' ),
            'button_label' => esc_html__( 'Support Forum', 'bosa-beauty-shop' ),
            'button_link' => esc_url( 'https://bosathemes.com/user-login-register' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'fifth' => array(
            'title' => esc_html__( 'Got sales related question?', 'bosa-beauty-shop' ),
            'desc' => esc_html__( "Have any query before purchase, you are more than welcome to ask.", 'bosa-beauty-shop' ),
            'button_label' => esc_html__( 'Pre-sale Question?', 'bosa-beauty-shop' ),
            'button_link' => esc_url( 'https://bosathemes.com/pre-sale' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'sixth' => array(
            'title' => esc_html__( 'Customization Request', 'bosa-beauty-shop' ),
            'desc' => esc_html__( 'Needed any customization for the theme, you can request from here.', 'bosa-beauty-shop' ),
            'button_label' => esc_html__( 'Customization Request', 'bosa-beauty-shop' ),
    ...
    'free_pro' => array (
        'first'=> array(
            'title' => esc_html__( 'Free VS Pro Features', 'bosa-beauty-shop' ),
            'desc' => sprintf( esc_html__( '%s is a multipurpose free WordPress theme. However, Bosa Pro comes with more cool and awesome features that give you a flexible and wide range of options to build a full-fledged website.', 'bosa-beauty-shop' ), $theme_name_config ),
            'link_title' => esc_html__( 'Check All Features', 'bosa-beauty-shop' ),
    ...
    'demos' => array (
        'first'=> array(
            'title' => esc_html__( 'Pre-built Demos', 'bosa-beauty-shop' ),
            'desc' => esc_html__ ( 'To get started with ready-made starter site templates.', 'bosa-beauty-shop' ),
            'link_title' => esc_html__( 'View All Demos', 'bosa-beauty-shop' ),
    ...
    'rating' => array(
        'first'=> array(
            'title' => esc_html__( 'Show us how much you like our theme.', 'bosa-beauty-shop' ),
            'desc' => esc_html__( 'If you like our work, please give us a moment of your time to rate our theme 5 stars on ', 'bosa-beauty-shop' ) .'<a target="_blank" href="https://wordpress.org/support/theme/' .$theme_name_config_url. '/reviews">' . 'wordpress.org' . '</a>' . esc_html__('. It will give us more energy to work on this theme.', 'bosa-beauty-shop' ),
            'link_title' => esc_html__( 'Rate Theme', 'bosa-beauty-shop' ),
